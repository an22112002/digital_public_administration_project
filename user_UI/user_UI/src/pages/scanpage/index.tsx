...
export default function ScanPage({ kiosk = false }: { kiosk?: boolean }) {
  ...
  const handleWebSocketReceive = (data: any) => {
    ...
    if (isCropMessage) {
      ...
      const croppedImages: ScanFile[] = normalizeCroppedImagePaths(data)
...
function createScanFileFromPath(
  ...
  if (!match) {
    ...
    if (scannedFileIndex >= 0) {
      return { url: imagePath, link: normalizedPath.slice(scannedFileIndex) };
    }
...
function normalizeCroppedImagePaths(value: unknown): string[] {
  if (Array.isArray(value)) return value.flatMap(item => normalizeCroppedImagePaths(item));
  if (typeof value === 'string') return [value];
  if (value && typeof value === 'object') {
    const response = value as Record<string, unknown>;
    const nestedImages = response.cropped_images ?? response.cropped_image ?? response.images ?? response.image ?? response.path ?? response.url ?? response.result ?? response.data;
