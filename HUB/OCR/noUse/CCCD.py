...
#     results = decode(image=Image.open(image_path), symbols=[ZBarSymbol.QRCODE])
...
def read_qr_code(image_path) -> dict | None:
    ...
    # Tìm và đọc tất cả QR trong ảnh
...
async def OCR_CCCD2(files: list[str], redis_client: Redis, id: str) -> dict | None:
    """
    Hàm thực hiện OCR cho CCCD từ hình ảnh.
    :param files: Danh sách đường dẫn đến các hình ảnh chứa CCCD.
    :return: Dictionary chứa thông tin CCCD hoặc None nếu không đọc được.
...
async def OCR_CCCD3(files: list[str], redis_client: Redis, id: str) -> dict | None:
    ...
    await redis_client.lpush(
        "ocr:jobs",
        json.dumps({
            "job_id": f"ocr_003_{id}",
            "images_path": files,
        })
    )
    message = await redis_client.blpop(
        "ocr:results:" + f"ocr_003_{id}",
        timeout=0
    )
    if message is None:
        return None
    _, result_json = message
    result = json.loads(result_json)
    if result.get("job_id") != f"ocr_003_{id}":
        return None
    if result.get("success") != True:
        return None
    strings = []
